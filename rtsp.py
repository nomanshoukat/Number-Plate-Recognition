...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description='Read license plates from a RTSP stream and save the result in a CSV file.',
        epilog='For example:python anpr_camera_stream.py --regions de --output G:\get\csv\save.csv'
    )
...
def capture(args, writer):
    ...
    while camera.capture.isOpened():
        ...
        response = requests.post('https://api.platerecognizer.com/v1/plate-reader',
                                 ...
                                 data=dict(regions=args.regions or 'gn'),
